...
public class MockExamServiceImpl implements MockExamService{
	...
	public ExamResultDTO submitExam(Long examId, SubmitExamDTO submitExamDTO) {
	    ...
	    StudentExamAttempt attempt = studentExamAttemptRepository
	            .findById(submitExamDTO.getAttemptId())
	            .orElseThrow(() -> new ResourceNotFoundException(
	                    "Exam attempt not found with id: " + submitExamDTO.getAttemptId()));

	    if (!attempt.getUser().getId().equals(user.getId())) {
	        throw new BusinessException(
	                "Exam attempt does not belong to current user");
	    }
	    ...
	    List<MockExamQuestion> examQuestions =
	            mockExamQuestionRepository.findByMockExamId(examId);

	    Set<Long> examQuestionIds = examQuestions.stream()
	            .map(mockExamQuestion -> mockExamQuestion.getQuestion().getId())
	    ...
	    for (ExamAnswerDTO answerDTO : submitExamDTO.getAnswers()) {
	        ...
	        boolean correct =
	                ...
	                && answerDTO.getAnswer().equalsIgnoreCase(
	    ...
	    boolean passed =
	            score >= attempt.getMockExam().getPassingScore();
	...
	public void addQuestionToExam(Long examId, AddQuestionToExamDTO dto) {
		...
		MockExam mockExam = mockExamRepository.findById(examId)
	            .orElseThrow(() -> new ResourceNotFoundException(
	                    "Mock exam not found with id: " + examId));
